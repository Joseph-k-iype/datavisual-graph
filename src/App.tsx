...
function App() {
  ...
  const handleViewLineage = (node: Node) => {
  ...
  const handleLayoutChange = async (type: 'hierarchical' | 'circular' | 'grid' | 'force') => {
    setLayoutType(type);
    const { nodes: layouted, edges: layoutedEdges } = await applyLayout(nodes, edges, {
      type,
      direction: 'DOWN',
      spacing: 120,
      layerSpacing: 180,
    });
    // Note: You'd need to update nodes/edges state here
