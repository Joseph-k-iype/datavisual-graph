// frontend/src/components/EnhancedLineageGraph.tsx
...
function convertToFlowNodes(
  ...
  // Separate schema classes and data instances
  ...
  // Position schema classes horizontally
  ...
  // Position data instances under their parent classes
    ...
    instances.forEach((instance, index) => {
      ...
      nodes.push({
        ...
        position: instance.position || {
          x: parentX + (index % 3) * 200 - 200, // Arrange in rows of 3
  ...
  return (
    ...
    >
      ...
      {data.data?.description && (
        <p className="text-xs text-gray-600 mb-2">{data.data.description}</p>
      )}

      <div className="flex items-center justify-between text-xs text-gray-500 pt-2 border-t border-gray-200">
        <span className="flex items-center gap-1">
          {data.instance_count || 0} instances
          {hasInstances && data.collapsed && (
            <span className="text-blue-600 font-medium">(click to expand)</span>
          )}
        </span>
        {data.attributes && data.attributes.length > 0 && (
          <span>{data.attributes.length} attrs</span>
        )}
      </div>
    </div>
  );
